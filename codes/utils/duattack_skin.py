    ...
    def DD_label_m1(self, x, mask, y=None, targeted=False,
        ...
        for i in range(iteration):
            ...
            with torch.no_grad():
                ...
                left_preds = left_preds.to(device) - 1
            # check
            if targeted:
                l_remaining = left_preds.ne(y)
                l_loss = nn.functional.cross_entropy(left_out, y+1, reduction='sum')
            else:
                l_remaining = left_preds.eq(y)
                l_loss = 3000.0 - nn.functional.cross_entropy(left_out, y+1, reduction='sum')
            ...
            with torch.no_grad():
                ...
                right_preds = right_preds.to(device) - 1
            # check
            if targeted:
                r_remaining = right_preds.ne(y)
                r_loss = nn.functional.cross_entropy(right_out, y+1, reduction='sum')
            else:
                r_remaining = right_preds.eq(y)
                r_loss = 3000.0 - nn.functional.cross_entropy(right_out, y+1, reduction='sum')
